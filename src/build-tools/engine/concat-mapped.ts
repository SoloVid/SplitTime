...
export async function concatEngineSource(): Promise<void> {
    ...
    const enginePromise = runAfter(() => concatEngine(), engineWithoutDomPromise)
    ...
    const editorServerPromise = runAfter(() => concatEditorServer(), engineWithoutDomPromise, editorServerLibPromise)
    const editorClientPromise = runAfter(() => concatEditorClient(), enginePromise, editorServerLibPromise)
    await Promise.all([
...
async function concatEngineWithoutDom() {
    ...
    ].concat(glob.sync('build/tsjs/*.js')
        .filter(p => p !== 'build/tsjs/defer.run.js'))
    .concat([
        'build/tsjs/engine/**/*.js'
    ])
    return concatFilesWithSourceMaps(files, 'build/engine-without-dom-libraries.js');
}
...
async function concatEditorServerLib() {
    const files = glob.sync('build/tsjs/editor/server/**/*.js')
...
async function concatEditorServer() {
    ...
    await fs.writeFile(generatedFile,
