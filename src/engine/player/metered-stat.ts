...
export class MeteredStat {
    private readonly emptyListeners = new RegisterCallbacks();
    private readonly maxedListeners = new RegisterCallbacks();
    constructor(public max: number = 0, public current: number = 0) { }
    registerEmptyListener(listener: () => CallbackResult) {
        this.emptyListeners.register(listener);
    }
    registerMaxedListener(listener: () => CallbackResult) {
