namespace SplitTime {
    ...
    export class WeatherRenderer {
        ...
        constructor(private readonly camera: Camera, private readonly ctx: CanvasRenderingContext2D) {
        ...
        render(level: Level) {
            ...
            if(level.weather.darkness > 0) {
                ...
                for(const body of bodies) {
                    if(body.lightIntensity > 0) {
                        ...
                        var grd = this.buffer.context.createRadialGradient(xCoord, yCoord, 1, xCoord, yCoord, body.lightRadius);
                        grd.addColorStop(0, "rgba(255, 255, 255, " + (level.weather.darkness * body.lightIntensity) + ")");
                        grd.addColorStop(1, "rgba(255, 255, 255, 0)");
                        this.buffer.context.fillStyle = grd;
                        this.buffer.context.beginPath();
                        this.buffer.context.arc(xCoord, yCoord, 150, 0, 2 * Math.PI);
                ...
                this.buffer.context.fillStyle = "rgba(0, 0, 0, " + level.weather.darkness + ")";//"#000000";
                this.buffer.context.fillRect(0, 0, this.SCREEN_WIDTH, this.SCREEN_HEIGHT);
                
                //Render buffer
                this.ctx.drawImage(this.buffer.element, 0, 0, this.SCREEN_WIDTH, this.SCREEN_HEIGHT);
            ...
            if(level.weather.lightningFrequency > 0) {
                ...
                if(SLVD.randomInt(SplitTime.FPS * 60) <= level.weather.lightningFrequency) {
        ...
        */
        private drawTiled(image: HTMLImageElement, left: number, top: number) {
            ...
            this.ctx.drawImage(image, left, top, this.SCREEN_WIDTH, this.SCREEN_HEIGHT, 0, 0, this.SCREEN_WIDTH, this.SCREEN_HEIGHT);
            
            var xEnd = image.naturalWidth - left;
            if(xEnd < this.SCREEN_WIDTH) {
                // Draw upper right tile if needed
                this.ctx.drawImage(image, 0, top, this.SCREEN_WIDTH, this.SCREEN_HEIGHT, xEnd, 0, this.SCREEN_WIDTH, this.SCREEN_HEIGHT);
            }
            
            var yEnd = image.naturalHeight - top;
            if(yEnd < this.SCREEN_HEIGHT) {
                // Draw lower left tile if needed
                this.ctx.drawImage(image, left, 0, this.SCREEN_WIDTH, this.SCREEN_HEIGHT, 0, yEnd, this.SCREEN_WIDTH, this.SCREEN_HEIGHT);
            }
            
            if(xEnd < this.SCREEN_WIDTH && yEnd < this.SCREEN_HEIGHT) {
                // Draw lower right tile if needed
                this.ctx.drawImage(image, 0, 0, this.SCREEN_WIDTH, this.SCREEN_HEIGHT, xEnd, yEnd, this.SCREEN_WIDTH, this.SCREEN_HEIGHT);
            }
        }
    }
}
