...
export class Menu {
    ...
    handleMenu() {
        ...
        if (rawDirection !== null) {
            controlDirection = simplifyToCardinal(rawDirection);
        }
                ...
                if (isUnderUpperBound && isAboveLowerBound) {
                    ...
                    dxTest =
                        this.spec.point[prevPoint].x -
                            this.spec.point[iPoint].x;
                    if (!isLeft)
                        dxTest += this.view.width;
                    dyTest = Math.abs(this.spec.point[prevPoint].y -
                        this.spec.point[iPoint].y);
                    if (dxTest <= dxBest) {
                ...
                if (isUnderUpperBound && isAboveLowerBound) {
                    ...
                    dyTest =
                        this.spec.point[prevPoint].y -
                            this.spec.point[iPoint].y;
                    if (!isUp)
                        dyTest += this.view.height;
                    dxTest = Math.abs(this.spec.point[prevPoint].x -
                        this.spec.point[iPoint].x);
                    if (dyTest <= dyBest) {
