...
class DialogDrawing {
    ...
    constructor(public dialog: SpeechBubbleState, public incoming: boolean = true, public visibility: number = 0) {
...
export class Renderer {
    ...
    notifyFrameUpdate() {
        for (const drawing of this.dialogDrawings) {
            if (drawing.incoming) {
                drawing.visibility = approachValue(drawing.visibility, 1, 0.1);
                // TODO: remove temporary hackaround bad UX
                drawing.visibility = 1;
                // TODO: Maybe add event here for dialog fully visible
            }
            else {
                drawing.visibility = approachValue(drawing.visibility, 0, 0.1);
                // TODO: remove temporary hackaround bad UX
    ...
    render(view: View) {
        for (const drawing of this.dialogDrawings) {
            ...
            if (location) {
                this.sayFromBoardFocalPoint(view.see, {
                    x: location.x,
                    y: location.y,
                    z: location.z
                }, dialog.getLineForMeasurement().replace(elisionRegex, elision), dialog.getDisplayedCurrentLine().replace(elisionRegex, elision), dialog.speaker ?? "");
            }
            else {
                this.simpleMessage(view.see, dialog.getLineForMeasurement().replace(elisionRegex, elision), dialog.getDisplayedCurrentLine().replace(elisionRegex, elision));
            }
        }
    }
    private drawAwesomeRect(left: number, top: number, right: number, bottom: number, ctx: GenericCanvasRenderingContext2D, pointX?: number, pointY?: number) {
        const CURVE_RADIUS = 10;
        const TRI_CURVE_BUFFER = 2 * CURVE_RADIUS;
        const TRI_BASE_HALF = (IDEAL_TAIL_LENGTH * TRI_BASE_TO_TAIL_LENGTH) / 2;
        ...
        if (pointX !== undefined && pointY !== undefined) {
            horizontalMid = constrain((2 * pointX + left + right) / 4, left + TRI_CURVE_BUFFER, right - TRI_CURVE_BUFFER);
            verticalMid = constrain((2 * pointY + top + bottom) / 4, top + TRI_CURVE_BUFFER, bottom - TRI_CURVE_BUFFER);
    ...
    private simpleMessage(ctx: GenericCanvasRenderingContext2D, fullMessage: string, displayedMessage: string) {
        this.drawSpeechBubble(ctx, fullMessage, displayedMessage, (areaWidth, areaHeight) => {
            return {
    ...
    private sayFromBoardFocalPoint(ctx: GenericCanvasRenderingContext2D, focalPoint: {
        x: number;
        y: number;
        z: number;
    }, fullMessage: string, displayedMessage: string, speakerName: string) {
        var pointRelativeToScreen = this.camera.getRelativeToScreen(focalPoint);
        this.drawSpeechBubble(ctx, fullMessage, displayedMessage, (areaWidth, areaHeight) => this.calculateDialogPosition(areaWidth, areaHeight, pointRelativeToScreen.x, pointRelativeToScreen.y), speakerName);
    }
    private drawSpeechBubble(ctx: GenericCanvasRenderingContext2D, fullMessage: string, displayedMessage: string, calculatePosition: PositionCalculator, speakerName?: string) {
        ...
        const maxTextWidth = this.calculateIdealizedMaxWidth(ctx, fullMessage, lineHeight, nameWidth);
        const lines = this.getLinesFromMessage(fullMessage, displayedMessage, ctx, maxTextWidth);
        const bubbleWidth = Math.max(lines.maxWidth, nameWidth) + 2 * TEXT_BOX_PADDING;
        const wholeBubbleTextHeight = lines.all.length * lineHeight;
        const bubbleHeight = wholeBubbleTextHeight + 2 * TEXT_BOX_PADDING;
        const position = calculatePosition(bubbleWidth, bubbleHeight + nameBoxHeight);
        let nameBoxShouldBeTop = true;
        if (isPositionPointed(position)) {
            nameBoxShouldBeTop = position.triPointY > position.top;
        }
        const messageTop = nameBoxShouldBeTop
        ...
        if (isPositionPointed(position)) {
            ...
            this.drawAwesomeRect(position.left, messageTop, position.left + bubbleWidth, messageTop + bubbleHeight, ctx);
        }
        //Lines
        ...
        if (CONFIG.SPEAKER_NAMES_ENABLED && speakerName) {
            var nameTop = nameBoxShouldBeTop
                ? position.top
    ...
    private drawText(ctx: GenericCanvasRenderingContext2D, text: string, x: number, y: number) {
    ...
    private calculateIdealizedMaxWidth(ctx: GenericCanvasRenderingContext2D, fullMessage: string, lineHeight: number, nameWidth: number): number {
        var singleLineWidth = ctx.measureText(fullMessage).width;
        var singleLineArea = singleLineWidth * lineHeight;
        var proposedWidth = Math.sqrt(singleLineArea / IDEAL_HEIGHT_TO_WIDTH) +
            3 * lineHeight;
        return Math.min(MAX_ROW_LENGTH, Math.max(MIN_ROW_LENGTH_SPLIT, nameWidth, proposedWidth));
    }
    private calculateDialogPosition(areaWidth: number, areaHeight: number, focalPointX: number, focalPointY: number): PointedDialogPosition {
        // Start centered (around focal point) horizontally
        var idealLeft = constrain(focalPointX - areaWidth / 2, // ideal
        ...
        if (top < MIN_SCREEN_MARGIN) {
            ...
            if (top + areaHeight >
                ...
                var idealTop = constrain(focalPointY - areaHeight / 2, // ideal
    ...
    private getLinesFromMessage(fullMessage: string, displayedMessage: string, ctx: GenericCanvasRenderingContext2D, maxRowLength: number): {
        maxWidth: number;
        all: string[];
        displayed: string[];
    } {
...
type PositionCalculator = (areaWidth: number, areaHeight: number) => DialogPosition | PointedDialogPosition;
