...
export class SpeechBubbleState implements Interruptible {
    ...
    constructor(private _line: string, private readonly timeline: Timeline, public readonly speaker?: string, private readonly location?: ILevelLocation2) {
    ...
    getDisplayedCurrentLine() {
        return this._effectiveLine.substr(0, Math.min(this._pausedAtChar + 1, this._effectiveLine.length));
    }
    notifyFrameUpdate() {
        let keepGoing = true;
