...
export class ConversationConnoisseur {
    ...
    constructor(private readonly perspective: LimitedPerspective) { }
    ...
    calculateContinuingConversationScore(conversation: ConversationInstance): number {
        ...
        const speakersExcludingPlayer = allSpeakers.filter(s => s.body !== this.perspective.playerBody);
        ...
        const distance = speakersToConsider
            .map(s => distanceEasy(focusPoint.x, focusPoint.y, s.body.x, s.body.y))
            .reduce((tempMin, tempDist) => Math.min(tempMin, tempDist), MAX_SAFE_INTEGER);
    ...
    calculateLineImportanceScore(lineSpeechBubble: LineSpeechBubble): number {
        ...
        const focusPoint = this.perspective.camera.getFocusPoint();
        const distance = !!location ? distanceEasy(focusPoint.x, focusPoint.y, location.x, location.y) : 1;
        const distanceScore = this.perspective.camera.SCREEN_WIDTH /
            3 /
