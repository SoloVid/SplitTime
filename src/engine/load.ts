namespace SplitTime {
	...
	export function launch(width: int = 640, height: int = 360, parentId?: string, additionalCanvasClass: string = "") {
		...
		const perspective = new player.Perspective(WORLD, view);
		...
		for(i = 0; i < masterData.levels.length; i++) {
			...
			promiseCollection.push(level.load(WORLD, levelData).then(incrementAndUpdateLoading));
		}
