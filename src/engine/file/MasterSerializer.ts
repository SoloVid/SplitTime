namespace SplitTime.file {
    export class MasterSerializer {
        ...
        private objectSerializers: { id: string; serializer: ObjectSerializer<any> }[] = [];
        private specialSerializers: { id: string; serializer: SpecialSerializer }[] = [];
        ...
        registerObjectSerializer<T>(id: string, s: ObjectSerializer<T>): void {
            for(const item of this.objectSerializers) {
                if(item.id === id) {
                    throw new Error("Object serializer \"" + id + "\" is already registered");
                }
            }
        ...
        registerSpecialSerializer(id: string, s: SpecialSerializer): void {
            for(const item of this.specialSerializers) {
                if(item.id === id) {
                    throw new Error("Special serializer \"" + id + "\" is already registered");
                }
            }
