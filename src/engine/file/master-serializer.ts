...
import { SpecialSerializer } from "./special-serializer";
import { ObjectSerializer } from "./object-serializer";
import { jsonable } from "./json";
import { MasterSerialization } from "./master-serialization";
import { MasterDeserialization } from "./master-deserialization";
export class MasterSerializer {
    ...
    registerSpecialSerializer(s: SpecialSerializer): void {
        for (const item of this.specialSerializers) {
            if (item.id === s.id) {
                throw new Error('Special serializer "' + s.id + '" is already registered');
            }
        }
