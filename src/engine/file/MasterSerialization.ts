namespace SplitTime.file {
    export class MasterSerialization implements AnySerializer {
        ...
        constructor(
            private readonly objectSerializers: { id: string; serializer: ObjectSerializer<unknown> }[]
        ) {
        ...
        serialize<O extends object, T>(thing: O): int {
            ...
            for(const s of this.objectSerializers) {
                ...
                if(st.isT(thing)) {
                    ...
                    if(typeof bucket !== "object") {
                        throw new Error("Missing data bucket for serialized objects of type \"" + s.id + "\"");
                    }
                    if(id in bucket) {
                        throw new Error("There appears to be an object of type \"" + s.id + "\" already serialized with ID " + id);
                    }
            ...
            throw new Error("Unable to find object serializer for object of type " + thing.constructor.name + ": " + JSON.stringify(thing));
        }
    }
}