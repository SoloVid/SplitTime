namespace SplitTime.file {
    class DeserializeSecondPass implements AnyDeserializer {
        ...
        private resolveSinglePromise(objectId: int, promise: SLVD.Promise): void {
    ...
    export class MasterDeserialization {
        ...
        constructor(
            rawData: jsonable,
            objectSerializers: { id: string; serializer: ObjectSerializer<unknown> }[]
        ) {
            ...
            for(const s of objectSerializers) {
                ...
                for(const objectId in bucket) {
                    ...
                    const thing = s.serializer.deserialize(this.secondPass, json);
