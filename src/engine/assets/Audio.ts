...
namespace SplitTime.assets {
    ...
    export class Audio {
        ...
        registerSoundEffect(relativePath: string, loop = false) {
            this.registerAudio(SOUND_EFFECT_DIR + relativePath, relativePath, loop);
        };
        
        private registerAudio(relativePath: string, handle: string, loopAudio: boolean) {
            this.sounds[handle] = new HowlContainer(relativePath, handle, loopAudio);
        }
        ...
        */
        ...
        private crossFadeSimilar(sound: HowlContainer) {
            var similar = false;
            if(typeof this.currentBkgSound !== 'undefined' && sound.musicGroup == this.currentBkgSound?.musicGroup){
                similar = true;
                if(this.currentBkgSound?.howl && sound.howl) {
                    var timeOffset = this.currentBkgSound.howl.seek() as number || 0;
