...
export class Renderer {
    ...
    private _getBodyNode(graphNode: GraphBodyNode, body?: Body): BodyNode {
        ...
        throw new Error("Failed to find a suitable place in body rendering array for " +
            graphNode.ref);
    }
    render() {
     ...
     */
    private _fadeOccludingSprite(nodeInFront: BodyNode, nodeBehind: BodyNode): void {
        ...
        const overlappingPixels = Math.min(xDiffVal1, xDiffVal2, yDiffVal1, yDiffVal2);
        for (const drawable of nodeInFront.body.drawables) {
            ...
            if (drawable.playerOcclusionFadeFactor > 0.01) {
                ...
                if (ENABLED) {
                    if (drawable.playerOcclusionFadeFactor <
                        0 ||
                        drawable.playerOcclusionFadeFactor > 1) {
                        error("Sprite specified with playerOcclusionFadeFactor invalid value: " +
                ...
                if (!nodeInFront.body.fadeEnteringLevelPromise) {
                    ...
                    var pixelsFactor = crossFadeFactor *
                        drawable.playerOcclusionFadeFactor;
