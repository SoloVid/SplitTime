namespace SplitTime.body {
    ...
    function canSee(detective: Body, target: Body): boolean {
        ...
        if(doesRayReach(detective.speechBox, centerOfTarget, detective, target)) {
            return true;
        }
        // TODO: add more rays
        return false;
    ...
    function doesRayReach(start: ILevelLocation, end: ILevelLocation, detective: Body, target: Body): boolean {
        ...
        SLVD.bresenham.forEachPoint(start.getX(), start.getY(), end.getX(), end.getY(), (x, y) => {
            const collisionInfo = COLLISION_CALCULATOR.calculateVolumeCollision(start.getLevel(), x, 1, y, 1, start.getZ(), 1, detective);
