...
class InternalCollisionInfo implements ApiCollisionInfo {
    ...
    constructor(level: MinimalLevel, public targetOffset: PointerOffset | null) {
        this.zBlockedTopEx = level.lowestLayerZ;
    }
}
class CollisionCalculator {
    private pointerStack: string[] = [];
    getEventsInVolume(level: Level, startX: number, xPixels: number, startY: number, yPixels: number, startZ: number, zPixels: number): string[] {
        const eventTracesMap = {};
        level
            .getLevelTraces()
            .calculateVolumeEvents(eventTracesMap, Math.floor(startX), Math.ceil(xPixels), Math.floor(startY), Math.ceil(yPixels), Math.floor(startZ), Math.ceil(startZ + zPixels));
     ...
     */
    calculateVolumeCollision(collisionMask: CollisionMask, level: Level, startX: number, xPixels: number, startY: number, yPixels: number, startZ: number, zPixels: number, ignoreBodies: splitTime.Body[] = []): ApiCollisionInfo {
        ...
        function handleFoundBody(otherBody: splitTime.Body) {
        ...
        level.getCellGrid().forEachBody(collisionMask, startX, startY, startZ, startX + xPixels, startY + yPixels, startZ + zPixels, handleFoundBody);
        if (!collisionInfo.blocked) {
            ...
            if (traceCollision.blocked) {
                ...
                for (const pointerOffset of traceCollision.pointerOffsets) {
                    ...
                    try {
                        const otherLevelCollisionInfo = this.calculateVolumeCollision(collisionMask, pointerOffset.level, startX + pointerOffset.offsetX, xPixels, startY + pointerOffset.offsetY, yPixels, startZ + pointerOffset.offsetZ, zPixels);
                        // Note that the sign on the offset is flipped here
                        // because we are coming back from the other level's
                        // coordinates to our own.
                        const backTranslatedZBlockedTopEx = otherLevelCollisionInfo.zBlockedTopEx -
                            pointerOffset.offsetZ;
                        collisionInfo.zBlockedTopEx = Math.max(collisionInfo.zBlockedTopEx, backTranslatedZBlockedTopEx);
     ...
     */
    private calculateVolumeTraceCollision(level: Level, startX: number, xPixels: number, startY: number, yPixels: number, startZ: number, zPixels: number): InternalCollisionInfo {
        const solidCollisionInfo = new SolidCollisionInfo(level);
        level
            .getLevelTraces()
            .calculateVolumeSolidCollision(solidCollisionInfo, Math.floor(startX), Math.ceil(xPixels), Math.floor(startY), Math.ceil(yPixels), Math.floor(startZ), Math.ceil(startZ + zPixels));
        const pointerTraceInfo: PointerTraceInfo = {};
        level
            .getLevelTraces()
            .calculateVolumePointers(pointerTraceInfo, Math.floor(startX), Math.ceil(xPixels), Math.floor(startY), Math.ceil(yPixels), Math.floor(startZ), Math.ceil(startZ + zPixels));
...
export function chooseTheOneOrDefault<T>(map: {
    [id: string]: T;
}, defaultOption: T): T {
