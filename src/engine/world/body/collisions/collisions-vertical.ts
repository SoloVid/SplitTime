...
export class Vertical {
    ...
    zeldaVerticalMove(maxDZ: number): number {
        ...
        const collisionInfo = this.calculateZCollision(this.mover.body.level, this.mover.body.x, this.mover.body.y, this.mover.body.z, maxDZ);
        if (collisionInfo.dzAllowed !== 0) {
            ...
            if (isPointerOffsetSignificant(collisionInfo.targetOffset, this.mover.body.level)) {
     ...
     */
    calculateZCollision(level: Level, x: number, y: number, z: number, dz: number, ignoreBodies: splitTime.Body[] = []): VerticalCollisionInfo {
        ...
        for (var i = 0; i < steps; i++) {
            const originCollisionInfo = COLLISION_CALCULATOR.calculateVolumeCollision(this.mover.body.collisionMask, level, left, this.mover.body.width, top, this.mover.body.depth, lowerBoundZ, 1, ignoreBodies.concat(this.mover.body));
            ...
            lowerBoundZ += kHat;
            targetZ += kHat;
            const events = COLLISION_CALCULATOR.getEventsInVolume(level, left, this.mover.body.width, top, this.mover.body.depth, lowerBoundZ, 1);
    ...
    private canApplyCachedFall(level: Level, x: number, y: number, z: number, dz: number, ignoreBodies: splitTime.Body[] = []): boolean {
        if (dz >= 0) {
            return false;
        }
