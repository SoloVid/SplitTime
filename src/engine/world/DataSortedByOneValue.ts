...
SplitTime.DataSortedByOneValue = function(maxValue, bucketSize) {
...
SplitTime.DataSortedByOneValue.prototype._getBeyondMaxValue = function() {
    return this.maxValue + BUFFER;
};

SplitTime.DataSortedByOneValue.prototype.add = function(ref, object) {
...
SplitTime.DataSortedByOneValue.prototype.remove = function(ref) {
...
SplitTime.DataSortedByOneValue.prototype.resort = function(ref, value) {
...
SplitTime.DataSortedByOneValue.prototype.resortUpward = function(ref, value) {
    var currentIndex = this.reverseSortLookup[ref];
...
SplitTime.DataSortedByOneValue.prototype.resortDownward = function(ref, value) {
    var currentIndex = this.reverseSortLookup[ref];
    var oldObject = this.sortedByValue[currentIndex].object;
    var oldValue = this.sortedByValue[currentIndex].value;
 ...
 * @return {boolean}
 */
SplitTime.DataSortedByOneValue.prototype.forEachInRange = function(valueStart, valueEndEx, callback) {
    ...
    var iSortedEnd = index32 + 1 < this.valueLookup32.length ? this.valueLookup32[index32 + 1] : this.sortedByValue.length;
    var found = false;
    for(; iSorted < iSortedEnd; iSorted++) {
        var sortedItem = this.sortedByValue[iSorted];
        if(sortedItem.value >= valueStart && sortedItem.value < valueEndEx) {
