...
export class CellGrid {
    ...
    private _whereAreBodies: {
        [bodyRef: number]: WhereIsBody | undefined;
    };
    ...
    resort(body: splitTime.Body) {
        ...
        var whereIsBodyNow = new WhereIsBody(this, body);
        var me = this;
        function removeFromCell(body: splitTime.Body, x: int, y: int, z: int) {
        ...
        function addToCell(body: splitTime.Body, x: int, y: int, z: int) {
            var cell = me._grids[z][y * me._xCells + x];
            if (ENABLED) {
                for (var i = 0; i < cell.length; i++) {
                    if (cell[i] === body) {
     ...
     */
    private _adjustCellClaims(body: splitTime.Body, whitelistArea: WhereIsBody, blacklistArea: WhereIsBody, callback: (body: splitTime.Body, x: int, y: int, z: int) => void) {
        var iX, iY, iZ;
    ...
    forEachBody(collisionMask: CollisionMask, minX: number, minY: number, minZ: number, exMaxX: number, exMaxY: number, exMaxZ: number, callback: (arg0: splitTime.Body) => void) {
        var bodiesHit: {
            [bodyRef: number]: true;
        } = {};
        for (var iCellZ = this.getZIndex(minZ); iCellZ <= this.getZIndex(exMaxZ); iCellZ++) {
            for (var iCellY = this.getYIndex(minY); iCellY <= this.getYIndex(exMaxY); iCellY++) {
                for (var iCellX = this.getXIndex(minX); iCellX <= this.getXIndex(exMaxX); iCellX++) {
                    var cell = this._grids[iCellZ][iCellY * this._xCells + iCellX];
                    for (const body of cell) {
                        if (!bodiesHit[body.ref]) {
                            const isTargetCollisionLayer = !!(body.collisionMask.membership & collisionMask.search) ||
                                !!(collisionMask.membership & body.collisionMask.search);
                            if (isTargetCollisionLayer &&
    ...
    getZIndex(z: number): int {
        return Math.min(Math.max(0, Math.floor((z - this.level.lowestLayerZ) / PARTITION_SIZE)), this._zCells - 1);
    }
}
class WhereIsBody {
    ...
    constructor(cellGrid: level.CellGrid, body?: splitTime.Body) {
        if (body) {
...
function isXOverlap(minX: number, exMaxX: number, body: splitTime.Body) {
    var bodyLeft = body.getLeft();
    var noOverlap = exMaxX <= bodyLeft || bodyLeft + body.width <= minX;
    return !noOverlap;
}
function isYOverlap(minY: number, exMaxY: number, body: splitTime.Body) {
    var bodyTop = body.getTopY();
    var noOverlap = exMaxY <= bodyTop || bodyTop + body.depth <= minY;
    return !noOverlap;
}
function isZOverlap(minZ: number, exMaxZ: number, body: splitTime.Body) {
    var bodyBottom = body.z;
    var noOverlap = exMaxZ <= body.z || bodyBottom + body.height <= minZ;
    return !noOverlap;
}
