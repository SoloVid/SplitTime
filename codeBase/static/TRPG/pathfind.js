...
SLVDE.pathToEnemy = function(finder)
{
	//alert("start pf");
	...
	while(cont)
	{
		...
		{
			var currentAgent = SLVDE.boardAgent[index];
			if(!currentAgent.getTeam().isAllied(SLVDE.PF.finder.getTeam()))
			{
				//if(index > 100) alert("something is wrong");
				//If reached one opponent's position, set path to troop
				if(SLVDE.PF.pointC.x == SLVDE.xPixToTile(currentAgent.x) && SLVDE.PF.pointC.y == SLVDE.yPixToTile(currentAgent.y))
				{
					//alert("found target");
...
SLVDE.PF.evaluatePoint = function(x, y)
{
	...
	if(!SLVDE.PF.point[i] && SLVDE.currentLevel.layerFuncData[SLVDE.PF.finder.layer].data[i] != 255 && x >= 0 && y >= 0 && x < SLVDE.currentLevel.layerFunc[SLVDE.PF.finder.layer].width && y < SLVDE.currentLevel.layerFunc[SLVDE.PF.finder.layer].height)
	{
...
SLVDE.PF.evaluateSquare = function(x, y)
{
	...
	for(second = 0; second < SLVDE.PF.person.squares.length; second++)
	{
	...
	if(done != 1)
	{
		...
		for(second = 0; second < SLVDE.PF.person.oppTeam.length; second++)
		{
		...
		for(second = 0; second < SLVDE.PF.person.oppTeam[0].oppTeam.length; second++)
		{
